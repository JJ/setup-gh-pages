...
mkdir _layouts
perl -e '$_ = join("",<>);s/id="main_content">(.+)<\/section>/id="main_content">{{content}}<\/section>/sm;s/id="main_content" class="inner">(.+)<\/section>/ id="main_content" class="inner">{{content}}<\/section>/sm;s/ class="markdown-body">(.+)<\/article>/ class="markdown-body">{{content}}<\/article>/sm;s/<section>\s+<h1>(.+)<\/section>/<section>{{content}}<\/section>/sm;print;' index.html > _layouts/index.html
echo -e "---\nlayout: index\n---\n" > index.md
